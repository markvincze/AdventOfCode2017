...
// Part 1
var sum = rows.Select(r => r.Split(new char[] {'\t'}, StringSplitOptions.RemoveEmptyEntries).Select(n => Int32.Parse(n)).ToArray())
    .Select(r => r.Max() - r.Min())
    .Sum();
...
// Part 2
int sum2 = 0;
foreach(var row in rows.Select(r => r.Split(new char[] {'\t'}, StringSplitOptions.RemoveEmptyEntries).Select(n => Int32.Parse(n)).ToArray()))
{
