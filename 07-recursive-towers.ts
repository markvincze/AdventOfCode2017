...
const parseLine: (line: string) => TowerInfo = (line: string) => {
    const name = line.substring(0, line.indexOf(' '));
    // tslint:disable-next-line:radix
    const weight = parseInt(line.substring(line.indexOf('(') + 1, line.indexOf(')')));
    let next;
    if (line.indexOf('>') !== -1) {
        next = line.substr(line.indexOf('>') + 2).split(', ');
    } else {
        next = [];
    }
...
const cache = {};
const parsedInputs = input.map(i => parseLine(i));
...
for (const data of parsedInputs) {
    let node = cache[data.name];
    ...
    if (!node) {
        node = cache[data.name] = {
            ...
            isRoot: true,
            weight: data.weight,
            children: data.next.map(n => {
                let childNode = cache[n];
...
let root;
for (const data of parsedInputs) {
    if (cache[data.name].isRoot) {
        root = cache[data.name];
        console.log('Root:', data.name);
    }
}

const calcTotalWeight = (node: TowerNode) => {
...
const isBalanced = (node: TowerNode): boolean => {
    let weight = null;
    for (let child of node.children) {
        if (weight === null) {
            weight = child.totalWeight;
        } else if (weight !== child.totalWeight) {
            return false;
        }
    }
